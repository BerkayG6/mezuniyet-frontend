  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.user && response.data.token) {
        ...
        if (userRole === "yok") {
          navigate("/yok-predict");
        } else if (userRole === "student") {
          console.warn(`User with role ${userRole} logged in from YÖK page.`);
          navigate("/student-login");
        } else if (userRole === "university") {
          console.warn(`User with role ${userRole} logged in from YÖK page.`);
          navigate("/university-login");
        } else {
          setMessage("Giriş başarılı, ancak bilinmeyen kullanıcı rolü.");
          navigate("/");
        }
      } else {
        setMessage("Giriş başarısız: Geçersiz kimlik bilgileri.");
      }
    } catch (error) {
