...
drop_columns = ['DEPARTMENT_ID', 'UNIVERSITY_ID', "FACULTY_NAME","DEPARTMENT_NAME",
                "SCHOLARSHIP_TYPE_ID","SCORE_TYPE_ID","YEAR_ID","GENERAL_ENROLLMENT","GENERAL_CAPACITY"]
...
print("MAE:", mae)
print("R2 Score:", r2)
#MAE: 0.06982786364837083
#R2 Score: 0.9091195456216588

from sklearn.model_selection import cross_val_score
scores = cross_val_score(best_xgb, X, y, cv=5, scoring='neg_mean_absolute_error')
print("Cross-Validation MAE Scores:", -scores)
print("Mean MAE:", -scores.mean())
#Cross-Validation MAE Scores: [0.07766208 0.0616782  0.05974465 0.10859537 0.08583796]

data_2025=data_2025.drop(columns=['UNIVERSITY_ID', "FACULTY_NAME","DEPARTMENT_NAME",
                "SCHOLARSHIP_TYPE_ID","SCORE_TYPE_ID","GENERAL_ENROLLMENT","GENERAL_CAPACITY"])
...
#scaler olması gerekiyor sıkıntı çıktı burada
wannaPredict = result.loc[100110045].drop("ENROLLMENT_CHANGE_RATE")
...
pivot_df=pivot_df.drop(columns=["DEPARTMENT_ID","ds","ENROLLMENT_CHANGE_RATE"])


# Yeni tabloyu görüntüleme
print(pivot_df.head())

pivot_df=pivot_df.loc[:,["GENERAL_CAPACITY","GENERAL_ENROLLMENT",	"SUM_CAPACITY"	,"SUM_ENROLLMENT"	,"FIELD_RATE",	"LAST_SCORE",	"LAST_RANK",	"FIRST_SCORE","FIRST_RANK","DEPARTMENT_POPULARITY","SCORE_DIFFERENCE","AVERAGE_FIELD_RATE","CAPACITY_CHANGE_RATE","UNIVERSITY_POPULARITY","FIELD_RATE_CHANGE","LAST_RANK_CHANGE","FIRST_RANK_CHANGE"
]]

# Tahmin verilerini ölçeklendirme
pivot_df_scaled = scaler.fit_transform(pivot_df)


# Tahmin yapma
predictions_2025 = best_xgb.predict(pivot_df_scaled)
