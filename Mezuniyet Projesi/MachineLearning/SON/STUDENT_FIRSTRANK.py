...
X = data.drop(columns=['FIRST_RANK'])  # Bağımsız değişkenler
y = data['FIRST_RANK']  # Hedef değişken
...
#Logaritmik olarak y değerlerini çeviriyoruz
...
A=data_24.drop(columns=["FIRST_RANK"])
B=data_24["FIRST_RANK"]
...
lastTable = pd.concat([B.reset_index(), pd.Series(predict, name="Predicted_FIRST_RANK")], axis=1)
...
wannaPredict = result.loc[wannaDepartment].drop("FIRST_RANK")
...
print("Seçilen bölümün diğer yıllardaki verileri: ",selected_data[['YEAR_ID', 'FIRST_RANK']])
