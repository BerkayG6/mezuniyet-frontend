...
drop_columns = ['SCORE_TYPE_ID', 'UNIVERSITY_ID', "FACULTY_NAME","DEPARTMENT_NAME","SCHOLARSHIP_TYPE_ID"]
...
X = data.drop(columns=['CAPACITY_CHANGE_RATE',"ENROLLMENT_CHANGE_RATE"])  # Bağımsız değişkenler
y = data[['CAPACITY_CHANGE_RATE',"ENROLLMENT_CHANGE_RATE"]] # Hedef değişken
...
# Aykırı verileri tespit etmek için bir eşik belirleyin
threshold = 1.5  # Çok büyük farkları aykırı olarak kabul edebilirsiniz
outliers = latest_data[abs(latest_data['CAPACITY_CHANGE_RATE'] - latest_data['CAPACITY_CHANGE_RATE_PRED']) > threshold]

# Görselleştirme: Aykırı ve normal veriler
plt.figure(figsize=(12, 6))

# Normal veriler
plt.scatter(
    latest_data.index,
    latest_data['CAPACITY_CHANGE_RATE'],
    color='blue',
...
plt.scatter(
    outliers.index,
    outliers['CAPACITY_CHANGE_RATE'],
    color='red',
    label='Aykırı Veriler',
    alpha=0.8
)

plt.axhline(y=threshold, color='green', linestyle='--', label='Pozitif Eşik')
plt.axhline(y=-threshold, color='green', linestyle='--', label='Negatif Eşik')

plt.title('Kapasite Değişim Oranları ve Aykırı Değerler')
plt.xlabel('Veri Indexi')
plt.ylabel('Kapasite Değişim Oranı')
plt.legend()
plt.grid(True)
plt.show()
